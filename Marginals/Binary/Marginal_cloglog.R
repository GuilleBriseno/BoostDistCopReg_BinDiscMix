...
BernoulliCloglog_Mar <- function(loc = NULL, offset_mu = NULL){
  ...
  l_generic <- list(cdf = cdf_gen,
                    cdf_mixbincont = cdf_gen_mixresp)
  ...
  # PDF is not required here
  # cdf
  cdf_mu  <- expr( 1 - exp( -exp(f) ) ) 
  # derivative cdf
  dercdf.deretamu  <- expr( exp( -exp(f) ) * exp(f) )   
  
  
  # mixed responses
  cdf_mu_mixresp  <- expr( exp( -exp(f) ) ) 
  # derivative cdf 
  dercdf.deretamu_mixresp  <- expr( -( exp( -exp(f) ) * exp(f) ) )  
  ...
  response_mu <- function(f) 1 - exp( -exp(f) ) 
  ...
  if(loc == 1){
    ...
    offset_mu <- expr({
      if (!is.null(!!mu)) {
        ...
        RET <-  log( -log( 1 - temp1 ) )      # log(temp1/(1-temp1))
    ...
    offset_mu <- expr({
      if (!is.null(!!mu)) {
        ...
        RET <- log( -log( 1 - temp2 ) )       # log(temp2/(1-temp2))
        # Warning! Offset at zero is detrimental to the oobag approach to optimise mstop!
        #RET <- 0
      }
      return(RET)
  ...
  name_mu <- "Cloglog link function for binary responses: mu(cloglog link)"
  ...
  check_y_mu <- expr({
    if (!is.numeric(!!y))
      ...
      stop("response is not but ", sQuote("CloglogMargin()"))
    if (any(!!y < 0))
      #stop("response is not positive but ", sQuote("LogNormLSS()"))
      stop("response is not positive but ", sQuote("CloglogMargin()"))
    if (any(!!y > 1))
      stop("response is above 1 but ", sQuote("CloglogMargin()"))
    if ( any(unique(!!y) %in% c(0,1) == FALSE) )
      stop("response is not binary but ", sQuote("CloglogMargin()"))
    y
  })
