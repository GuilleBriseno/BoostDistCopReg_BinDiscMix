...
ZINBI_Mar <- function(loc = NULL, offset_mu = NULL, offset_sigma = NULL, offset_nu = NULL, aux_mat = NULL, dim_aux_mat = NULL){
  ...
  # der pdf 
  derpdf1.deretamu   <- expr( derZINBI_PDF.dermu2(resp = !!y, param1 = exp(f), param2 = !!sigma, param3 = !!nu) * exp(f) ) 
  ...
  # derivative cdf 
  dercdf.deretamu <- expr( get_derCDF_3param_etaparam(func = derZINBI_PDF.dermu2, resp = !!y, par1 = exp(f), par2 = !!sigma, par3 = !!nu) * exp(f) )
  ...
  # der pdf
  derpdf1.deretasigma <- expr({ derZINBI_PDF.dersigma2(resp = !!y, param1 = !!mu, param2 = exp(f), param3 = !!nu) * exp(f) })
  
  # cdf
  cdf_sigma <- expr( pZINBI(q = !!y, mu = !!mu, sigma = exp(f), nu = !!nu) )  
  
  # der cdf
  dercdf.deretasigma <- expr( get_derCDF_3param_etaparam(func = derZINBI_PDF.dersigma2, resp = !!y, par1 = !!mu, par2 = exp(f), par3 = !!nu) * exp(f) )
  ...
  # der pdf
  derpdf1.deretanu <- expr({ derZINBI_PDF.dernu2(resp = !!y, param1 = !!mu, param2 = !!sigma, param3 = plogis(f)) *  ( exp(f) / (1 + exp(f))^2 ) })
  
  # cdf
  cdf_nu <- expr( pZINBI(q = !!y, mu = !!mu, sigma = !!sigma, nu = plogis(f)) )  
  
  # der cdf
  dercdf.deretanu <- expr( get_derCDF_3param_etaparam(func = derZINBI_PDF.dernu2, resp = !!y, par1 = !!mu, par2 = !!sigma, par3 = plogis(f)) * ( exp(f) / (1 + exp(f))^2 ) )
  ...
  offset_nu <- expr({
    if (!is.null(!!nu)) {
      ...
      #temp <- rep(((sum(!!y == 0)/length(!!y))+0.01), length(!!y))
      temp <- rep(max((sum((!!y)==0)/length(!!y))-0.1, .01), length(!!y))
