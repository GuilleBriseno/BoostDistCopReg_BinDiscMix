...
ZANBI_Mar <- function(loc = NULL, offset_mu = NULL, offset_sigma = NULL, offset_nu = NULL, aux_mat = NULL, dim_aux_mat = NULL){
  ...
  pdf_gen <- expr( dZANBI(x = !!y, mu = !!mu, sigma = !!sigma, nu = !!nu) )
  # cdf
  cdf_gen <- expr( pZANBI(q = !!y, mu = !!mu, sigma = !!sigma, nu = !!nu) )  
  ...
  ### mu functions
  # pdf
  pdf_mu  <- expr( dZANBI(x = !!y, mu = exp(f), sigma = !!sigma, nu = !!nu) )
  
  # der pdf 
  derpdf1.deretamu   <- expr( derZANBI_PDF.dermu2(resp = !!y, param1 = exp(f), param2 = !!sigma, param3 = !!nu) ) 
  
  
  # cdf
  cdf_mu  <- expr( pZANBI(q = !!y, mu = exp(f), sigma = !!sigma, nu = !!nu) )  
  
  # derivative cdf 
  #dercdf.deretamu <- expr( get_derCDF_3param_etaparam(func = derZANBI_PDF.dermu2, resp = !!y, par1 = exp(f), par2 = !!sigma, par3 = !!nu) )
  # derDISCDIST_PDF.derparam2_3PARAMS
  dercdf.deretamu <-  expr( derDISCDIST_PDF.derparam2_3PARAMS(func = derZANBI_PDF.dermu2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = exp(f), par2 = !!sigma, par3 = !!nu) )
  ...
  ### sigma functions
  # pdf
  pdf_sigma <- expr(  dZANBI(x = !!y, mu = !!mu, sigma = exp(f), nu = !!nu)   )
  
  # der pdf
  derpdf1.deretasigma <- expr({ derZANBI_PDF.dersigma2(resp = !!y, param1 = !!mu, param2 = exp(f), param3 = !!nu) })
  
  # cdf
  cdf_sigma <- expr( pZANBI(q = !!y, mu = !!mu, sigma = exp(f), nu = !!nu) )  
  
  # der cdf
  #dercdf.deretasigma <- expr( get_derCDF_3param_etaparam(func = derZANBI_PDF.dersigma2, resp = !!y, par1 = !!mu, par2 = exp(f), par3 = !!nu) )
  dercdf.deretasigma <- expr( derDISCDIST_PDF.derparam2_3PARAMS(func = derZANBI_PDF.dersigma2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = !!mu, par2 = exp(f), par3 = !!nu) )
  ...
  ### nu functions
  # pdf
  pdf_nu <- expr(  dZANBI(x = !!y, mu = !!mu, sigma = !!sigma, nu = plogis(f))   )
  
  # der pdf
  derpdf1.deretanu <- expr({ derZANBI_PDF.dernu2(resp = !!y, param1 = !!mu, param2 = !!sigma, param3 = plogis(f)) })
  
  # cdf
  cdf_nu <- expr( pZANBI(q = !!y, mu = !!mu, sigma = !!sigma, nu = plogis(f)) )  
  
  # der cdf
  #dercdf.deretanu <- expr( get_derCDF_3param_etaparam(func = derZANBI_PDF.dernu2, resp = !!y, par1 = !!mu, par2 = !!sigma, par3 = plogis(f)) )
  dercdf.deretanu <- expr( derDISCDIST_PDF.derparam2_3PARAMS(func = derZANBI_PDF.dersigma2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = !!mu, par2 = !!sigma, par3 = plogis(f)) )
  ...
  name_mu <- "Zero-Altered Negative Binomial distribution: mu(log link)"
  name_sigma <- "Zero-Altered Negative Binomial distribution: sigma(log link)"
  name_nu <- "Zero-Altered Negative Binomial distribution: nu(logit link)"
  ...
  check_y_mu <- expr({
    if (!is.numeric(!!y))
      stop("response is not but ", sQuote("ZANBI()"))
    if (any(!!y < 0))
      stop("response is not positive but ", sQuote("ZANBI()"))
    y
  })
