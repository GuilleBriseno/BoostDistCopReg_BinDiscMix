...
ZINBI_Mar <- function(loc = NULL, offset_mu = NULL, offset_sigma = NULL, offset_nu = NULL, aux_mat = NULL, dim_aux_mat = NULL){
  ...
  pdf_gen <- expr( dZINBI(x = !!y, mu = !!mu, sigma = !!sigma, nu = !!nu) )
  # cdf
  cdf_gen <- expr( pZINBI(q = !!y, mu = !!mu, sigma = !!sigma, nu = !!nu) )  
  ...
  ### mu functions
  # pdf
  pdf_mu  <- expr( dZINBI(x = !!y, mu = exp(f), sigma = !!sigma, nu = !!nu) )
  
  # der pdf 
  derpdf1.deretamu   <- expr( derZINBI_PDF.dermu2(resp = !!y, param1 = exp(f), param2 = !!sigma, param3 = !!nu) ) 
  
  
  # cdf
  cdf_mu  <- expr( pZINBI(q = !!y, mu = exp(f), sigma = !!sigma, nu = !!nu) )  
  
  # derivative cdf 
  #dercdf.deretamu <- expr( get_derCDF_3param_etaparam(func = derZINBI_PDF.dermu2, resp = !!y, par1 = exp(f), par2 = !!sigma, par3 = !!nu) )
  dercdf.deretamu <-  expr( derDISCDIST_PDF.derparam2_3PARAMS(func = derZINBI_PDF.dermu2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = exp(f), par2 = !!sigma, par3 = !!nu) )
  ...
  ### sigma functions
  # pdf
  pdf_sigma <- expr(  dZINBI(x = !!y, mu = !!mu, sigma = exp(f), nu = !!nu)   )
  
  # der pdf
  derpdf1.deretasigma <- expr({ derZINBI_PDF.dersigma2(resp = !!y, param1 = !!mu, param2 = exp(f), param3 = !!nu) })
  
  # cdf
  cdf_sigma <- expr( pZINBI(q = !!y, mu = !!mu, sigma = exp(f), nu = !!nu) )  
  
  # der cdf
  #dercdf.deretasigma <- expr( get_derCDF_3param_etaparam(func = derZINBI_PDF.dersigma2, resp = !!y, par1 = !!mu, par2 = exp(f), par3 = !!nu) )
  dercdf.deretasigma <- expr( derDISCDIST_PDF.derparam2_3PARAMS(func = derZINBI_PDF.dersigma2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = !!mu, par2 = exp(f), par3 = !!nu) )
  ...
  # der pdf
  derpdf1.deretanu <- expr({ derZINBI_PDF.dernu2(resp = !!y, param1 = !!mu, param2 = !!sigma, param3 = plogis(f)) })
  
  # cdf
  cdf_nu <- expr( pZINBI(q = !!y, mu = !!mu, sigma = !!sigma, nu = plogis(f)) )  
  
  # der cdf
  #dercdf.deretanu <- expr( get_derCDF_3param_etaparam(func = derZINBI_PDF.dernu2, resp = !!y, par1 = !!mu, par2 = !!sigma, par3 = plogis(f)) )
  dercdf.deretanu <- expr( derDISCDIST_PDF.derparam2_3PARAMS(func = derZINBI_PDF.dernu2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = !!mu, par2 = !!sigma, par3 = plogis(f)) )
  ...
  name_mu <- "Zero-Inflated Negative Binomial distribution: mu(log link)"
  name_sigma <- "Zero-Inflated Negative Binomial distribution: sigma(log link)"
  name_nu <- "Zero-Inflated Negative Binomial distribution: nu(logit link)"
  ...
  check_y_mu <- expr({
    if (!is.numeric(!!y))
      stop("response is not but ", sQuote("ZINBI()"))
    if (any(!!y < 0))
      stop("response is not positive but ", sQuote("ZINBI()"))
    y
  })
