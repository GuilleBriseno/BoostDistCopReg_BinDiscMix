...
ZIPoisson_Mar <- function(loc = NULL, offset_mu = NULL, offset_sigma = NULL, aux_mat = NULL, dim_aux_mat = NULL, aux_matrix = NULL){
  ...
  if ((!is.null(offset_sigma) && offset_sigma <= 0))
    stop(sQuote("sigma"), paste(" must be within (0,1) in Marginal", loc))

  if ((!is.null(offset_sigma) && offset_sigma >= 1))
    stop(sQuote("sigma"), paste(" must be within (0,1) in Marginal", loc))
  ...
  derpdf1.deretamu   <- expr({ ifelse( !!y > 0, 
                                       (1 - (!!sigma)) * ((exp(f))^(((!!y)) - 1) * ((!!y)))/gamma((!!y) + 1) * exp(-(exp(f))) - (1 - (!!sigma)) * (exp(f))^((!!y))/gamma((!!y) + 1) * exp(-(exp(f))), 
                                       -( (1 - (!!sigma)) * exp(-(exp(f))) ) * exp(f)
  ) })#expr({  derZIP_PDF.dermu2(resp = !!y, param1 = exp(f), param2 = !!sigma) })
  ...
  ### Sigma functions
  # pdf
  pdf_sig <- expr( dZIP(x = !!y, mu = !!mu, sigma = plogis(f)) )
  
  derpdf1.deretasigma <- expr({ ifelse((!!y) > 0, 
                                       -((!!mu)^((!!y))/gamma((!!y) + 1) * exp(-(!!mu))), 
                                       1 - exp(-(!!mu))) * ( ((plogis(f))/(1-(plogis(f)))*(1+(plogis(f))/(1-(plogis(f))))-((plogis(f))/(1-(plogis(f))))^2)/(1+(plogis(f))/(1-(plogis(f))))^2 ) })#expr({ derZIP_PDF.dersigma2(resp = !!y, param1 = !!mu, param2 = plogis(f))  })
  
  
  # cdf 
  cdf_sig <- expr( pZIP(q = !!y, mu = !!mu, sigma = plogis(f)) )
  
  # derivative cdf 
  dercdf.deretasig <-  expr({  get_derCDF_etaparam(func = derZIP_PDF.dersigma2, resp = !!y, par1 = !!mu, par2 = plogis(f)) })
  ...
  offset_sigma <- expr({
    ...
    if (!is.null(!!sigma)) {
      ...
      RET <- log(!!sigma / (1 - !!sigma) )#qlogis(!!sigma)
    
      }
    else {
      
      #temps <- rep(0.1, length(!!y) ) 
      
      
      #max( ( (weighted.sd(!!y, w = w, na.rm = TRUE) / weighted.mean(!!y, w = w, na.rm = TRUE))-1 ), 0.1)#, length(!!y) ) # taken from gamlss
      
      #temps <- rep(0.5, length(!!y))
      
      #  rep( max( ( (weighted.sd(!!y, w = w, na.rm = TRUE)^2 / weighted.mean(!!y, w = w, na.rm = TRUE))-1 ), 0.1), length(!!y) )
      
      # Two lines below were the previous offset
      #temps <- weighted.mean( 1*((!!y) == 0), w = w, na.rm = TRUE )
      #RET <- qlogis(temps)                                                 # but weighted version, taken from gamboostLSS
  ...
  name_sigma <- "Zero Inflated Poisson distribution: sigma(logit link)"
  ...
  check_y_mu <- expr({
    if (!is.numeric(!!y))
      stop("response is not but ", sQuote("ZeroInflatedPoisson()"))
    if (any(!!y < 0))
      stop("response is not positive but ", sQuote("ZeroInflatedPoisson()"))
    y
  })
  ...
  l = list(parameters = c(mu, sigma),
           ...
           name = l_names,
           check_y = l_check_y,
           marg_name = "ZeroInflatedPoissonMarg")
