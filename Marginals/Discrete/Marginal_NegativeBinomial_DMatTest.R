...
#### THE IDEA IS TO APPLY THE FUNCTION OF THE DERIVATIVE OF THE PDF TO EACH OF THE ENTRIES PER ROW OF AUX MAT!!!
...
NBI_Mar <- function(loc = NULL, offset_mu = NULL, offset_sigma = NULL, aux_mat = NULL, dim_aux_mat = NULL){
  ...
  dercdf.deretamu <-  expr( derDISCDIST_PDF.derparam2(func = derNBI_PDF.dermu2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = exp(f), par2 = !!sigma) )
  
  
  ### Change the function here to a test function derNBI_PDFTEST.dermu2(!!aux_mat[i,], par1 = exp(f), par2)
  #testderiv_mu <- expr( rowSums(t(sapply(1:(!!dim_aux_mat)[1], function(i) derPOISSON_PDF.dermu2(!!aux_mat[i,], exp(f)[i]))), na.rm = TRUE) * exp(f) )
  ...
  # der cdf
  # derDISCDIST_PDF.derparam2(func = derNBI_PDF.dermu2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = exp(f), par2 = !!sigma) 
  #dercdf.deretasigma <- expr( get_derCDF_etaparam(func = derNBI_PDF.dersigma2, resp = !!y, par1 = !!mu, par2 = exp(f)) )
  dercdf.deretasigma <- expr( derDISCDIST_PDF.derparam2(func = derNBI_PDF.dersigma2, auxmatrix = !!aux_mat, dim_aux_matrix = !!dim_aux_mat, resp = !!y, par1 = !!mu, par2 = exp(f)) )
