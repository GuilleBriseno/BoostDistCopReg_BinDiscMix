...
Clayton_Cop <- function(marg1 = NULL, marg2 = NULL,
...
gradient_marg_gen <- function(derlpdf1, cdf1, cdf2, dercdf1){
  expr(ngr <-  !!derlpdf1 - 
               (1 + rho)/(!!cdf1) * (!!dercdf1) + 
               (2 + 1/rho) * (rho * (!!cdf1)^(-rho -1))/((!!cdf1)^(-rho) + (!!cdf2)^(-rho) -1) * (!!dercdf1))
}

gradient_cop_gen <- function(cdf1, cdf2, rho){
  expr(ngr <- ((1)/(!!rho +1) - 
                 (log(!!cdf1) + log(!!cdf2)) + 
                 (log((!!cdf1)^(-(!!rho)) + (!!cdf2)^(-(!!rho)) - 1)/ (!!rho)^2) +
                 (2 + 1/(!!rho)) * (log(!!cdf1) * (!!cdf1)^(-(!!rho)) + log(!!cdf2) * (!!cdf2)^(-(!!rho))) / 
                    ((!!cdf1)^(-(!!rho)) + (!!cdf2)^(-(!!rho)) - 1)) *
         (exp(f)))
}
...
loss_gen <- function(pdf1, pdf2, cdf1, cdf2, rho){                                
  expr(-(log( !!pdf1 ) + log( !!pdf2 ) + 
         log(!!rho + 1) - (1 + !!rho) * (log(!!cdf1) + log(!!cdf2)) - (2 + 1/(!!rho)) * log((!!cdf1)^(-!!rho) + (!!cdf2)^(-!!rho) - 1)))    
}
...
# Clayton_Cop_Gauss_Magrs <- Clayton_Cop(marg1 = "LOGNO", marg2 = "LOGLOG")
# 
# Clayton_Cop_Gauss_Magrs
