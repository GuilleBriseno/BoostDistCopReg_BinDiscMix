...
Gumbel_Cop <- function(marg1 = NULL, marg2 = NULL,
  ...
  if(!(marg1 %in% c("NO", "SM", "LOGNO", "GA", "DAGUM", "LOGLOG"))) stop("First Marginal distribution not available.")
  if(!(marg2 %in% c("NO", "SM", "LOGNO", "GA", "DAGUM", "LOGLOG"))) stop("Second Marginal distribution not available.")
...
gradient_marg_gen <- function(derlpdf1, cdf1, cdf2, dercdf1){
  expr(ngr <-  !!derlpdf1 + 
         ( ((-log(!!cdf1))^rho + (-log(!!cdf2))^rho)^(1/rho - 1) * (-log(!!cdf1))^(rho - 1) * (1/!!cdf1) +
             (rho - 1) * (1 / -log(!!cdf1)) * (-1/!!cdf1) -
             (1/!!cdf1) +
             (1/rho - 2) * (1 / ((-log(!!cdf1))^rho + (-log(!!cdf2))^rho)) * rho * (-log(!!cdf1))^(rho - 1) * (-1/ !!cdf1) +
             (1 / (((-log(!!cdf1))^rho + (-log(!!cdf2))^rho)^(1/rho) + rho - 1)) * ((-log(!!cdf1))^rho + (-log(!!cdf2))^rho)^(1/rho - 1) * (-log(!!cdf1))^(rho - 1) * (-1/!!cdf1) ) *
           (!!dercdf1))
}


gradient_cop_gen <- function(cdf1, cdf2, rho){
  expr(ngr <- - (((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho))^(1/!!rho) * (-(!!rho - 1)/(!!rho)^2 * log((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho)) + 1/!!rho * ((-log(!!cdf1))^(!!rho) * log(-log(!!cdf1)) * (!!rho - 1) + (-log(!!cdf2))^(!!rho) * log(-log(!!cdf2)) * (!!rho - 1))/((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho)))) + 
         (!!rho - 1) * (log(-log(!!cdf1)) + log(-log(!!cdf2))) -
         (2 * (!!rho - 1))/(!!rho)^2 *log((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho)) + (2/!!rho-2) * ((-log(!!cdf1))^(!!rho) * log(-log(!!cdf1)) * (!!rho - 1) + (-log(!!cdf2))^(!!rho) * log(-log(!!cdf2)) * (!!rho - 1))/((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho)) +
         (((!!rho) - 1) * ((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho))^(-1/!!rho) + (!!rho -1) * (((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^!!rho)^(-1/!!rho) * ((!!rho - 1)/(!!rho)^2 * log((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho)) - 1/!!rho * ((-log(!!cdf1))^(!!rho) * log(-log(!!cdf1)) * (!!rho - 1) + (-log(!!cdf2))^(!!rho) * log(-log(!!cdf2)) * (!!rho - 1))/((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho))))) / (1 + (!!rho - 1) * ((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho))^(-1/!!rho)))
...
loss_gen <- function(pdf1, pdf2, cdf1, cdf2, rho){                                
  expr(-(log( !!pdf1 ) + log( !!pdf2 ) -
        ((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho))^(1/!!rho) + 
         (!!rho-1)*(log(-log(!!cdf1)) + log(-log(!!cdf2))) - 
         log(!!cdf1) - log(!!cdf2) +
         (2/!!rho - 2) * log((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho)) +
         log(1 + (!!rho - 1) * ((-log(!!cdf1))^(!!rho) + (-log(!!cdf2))^(!!rho))^(-1/!!rho))))
...
# Gumbel_Cop_Gauss_Magrs <- Gumbel_Cop(marg1 = "LOGNO", marg2 = "LOGNO")
# Gumbel_Cop_Gauss_Magrs$mu1@ngradient
