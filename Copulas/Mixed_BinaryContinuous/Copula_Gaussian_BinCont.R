...
Gauss_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_margin1_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      HFunc <- pdffz( pnorm( (qnorm(F1) - (rho) * qnorm(F2))/sqrt(1 - (rho)^2) ) )
      
      ####################
      dHFunc_dF1 <-  ( 1/sqrt(1 - rho^2)*exp(  - (rho^2*( qnorm(F1)^2 +  qnorm(F2)^2 ) - 2*rho*qnorm(F1)*qnorm(F2) ) / (2*(1 - rho^2)) ) ) 
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2, derlpdf2){
    ...
    expr({
      ...
      ################## Partial derivatives
      dHFunc_dF2 <- dnorm((qnorm(F1) - rho*qnorm(F2))/sqrt(1 - rho^2))  * sqrt(2*pi) *(-rho)/sqrt(1 - rho^2)/exp(-qnorm(F2)^2/2) 
  ...
  expr({
    ...
    ################### Partial derivatives:
    dHfunc_dTheta <- (-(dnorm((qnorm(F1) - rho * qnorm(F2))/sqrt(1 - (!!rho)^2)) * (1/cosh(f)^2 * qnorm(F2)/sqrt(1 - (!!rho)^2) - (qnorm(F1) - rho * qnorm(F2)) * (0.5 * (2 * (1/cosh(f)^2 * rho) * (1 - (!!rho)^2)^-0.5))/sqrt(1 - (!!rho)^2)^2)))/(1/cosh(f)^2)  
  ...
  name_fam <- paste("GaussianCop (Mixed binary - continuous margins)", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Gauss copula with mixed binary and continuous margins: rho(... link)"                                       
