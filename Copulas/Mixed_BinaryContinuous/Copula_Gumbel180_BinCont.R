...
Gumbel180_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2, derlpdf2){
    ...
    expr({
      ...
      ################## Partial derivatives
      dHFunc_dF2 <- (1/(F2^2))*exp(-((-log(F2))^thet + (-log(F1))^thet)^((1/thet)))* (-log(F2))^(-2 + thet) *((-log(F2))^thet + (-log(F1))^thet)^(-2 + 1/thet)* ((-log(F2))^thet* (log(F2) + ((-log(F2))^thet + (-log(F1))^thet)^(1/thet)) + (1 - thet + log(F2))* (-log(F1))^thet) 
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      ################### Partial derivatives:
      dHfunc_dTheta <- (-1)*( ((-log(F2))^(-1 + thet) * (((-log(F1))^thet * log(-log(F1)) + (-log(F2))^thet * log(-log(F2))) * ((-log(F1))^thet + (-log(F2))^thet)^(1/thet - 2) * (1/thet - 1) - ((-log(F1))^thet + (-log(F2))^thet)^(1/thet - 1) * log((-log(F1))^thet + (-log(F2))^thet)/thet^2) + ((-log(F1))^thet + 
  ...
  name_fam <- paste("Gumbel180Cop (Mixed binary - continuous margins)", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Rotated Gumbel (180°) copula with mixed binary and continuous margins: rho(... link)"                                       
