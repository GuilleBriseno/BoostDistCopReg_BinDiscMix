...
Clayton180_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      ################### Partial derivatives:
      dHfunc_dTheta <- (-1)*( ((1 + 1/thet) * (log(F1)/F1^thet + log(F2)/F2^thet)/(1/F1^thet + 1/F2^thet - 1)^(1/thet + 2) + log(1/F1^thet + 1/F2^thet - 1)/(thet^2 * (1/F1^thet + 1/F2^thet - 1)^(1 + 1/thet)))/F2^(1 + thet) + (1/F2^(1 + thet) - (1/F2^(1 + thet) + thet * log(F2)/F2^(1 + thet)))/(thet * (1/F1^thet + 1/F2^thet - 1)^(1 + 1/thet)) )
  ...
  name_fam <- paste("ClaytonCop 180 degree rotation (Mixed binary - continuous margins)", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Rotated Clayton copula (180°) with mixed binary and continuous margins: rho(... link)"                                       
