...
Frank_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_margin1_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      ####################
      dHFunc_dF1 <- dvffz( (exp((1 + F1 + F2)* thet)* (-1 + exp(thet))* thet)/(exp((F1 + F2)* thet) - exp(thet)* (-1 + exp(F1* thet) + exp(F2* thet)))^2 )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2, derlpdf2){
    ...
    expr({
      ...
      ################## Partial derivatives
      dHFunc_dF2 <- dvffz( (exp(thet + F2* thet)* (-1 + exp(F1* thet))* (-exp(thet) + exp(F1* thet))* thet)/(exp((F1 + F2)* thet) - exp(thet)* (-1 + exp(F2* thet) + exp(F1* thet)))^2 )
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      ################### Partial derivatives:
      dHfunc_dTheta <- dvffz( (exp(thet + F2 *thet)* (exp(2* F1* thet)* (-1 + F2) + exp(thet)* F2 - exp(F1* thet)* (-1 + F2 + exp(thet)* (F2 - F1) + F1)))/(exp((F2 + F1)* thet) - exp(thet)* (-1 + exp(F2* thet) + exp(F1* thet)))^2 )
      
      ngr <- ( ( ( (1 - y[,1]) / HFunc ) - (y[,1] / ( pdffz(1 - HFunc) ) ) ) * dHfunc_dTheta * (1/cosh(f)^2)  )
