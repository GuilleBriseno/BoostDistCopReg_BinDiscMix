...
Clayton90_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_margin1_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      ngr <- ( ( ( (1-y[,1]) / HFunc ) - (y[,1] / (pdffz(1 - HFunc)) ) ) * dHFunc_dF1  * dF1_deta )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2, derlpdf2){
    ...
    expr({
      ...
      ################## Partial derivatives
      dHFunc_dF2 <-  (-1)*( (F2^(-2 + thet)* F1^thet* (-1 + F2^-thet + F1^-thet)^(-1/thet)* (-1 + F1^thet)* (1 + thet))/(F1^thet - F2^thet* (-1 + F1^thet))^2 ) 
  ...
  name_fam <- paste("ClaytonCop 90 degree rotation (Mixed binary - continuous margins)", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Rotated Clayton copula (90°) with mixed binary and continuous margins: rho(... link)"                                       
