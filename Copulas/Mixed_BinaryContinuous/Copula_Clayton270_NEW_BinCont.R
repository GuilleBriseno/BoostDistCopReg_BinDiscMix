...
Clayton270_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_margin1_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      ####################
      dHFunc_dF1 <- dvffz( F1^(-1 - thet)* F2^(-1 - thet)* (-1 + F1^-thet + F2^-thet)^(-2 - 1/thet) *(1 + thet) )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2, derlpdf2){
    ...
    expr({
      ...
      ################## Partial derivatives
      dHFunc_dF2 <-  dvffz(( (F2^(-2 + thet)* F1^thet* (-1 + F2^-thet + F1^-thet)^(-1/thet)* (-1 + F1^thet)* (1 + thet))/(F1^thet - F2^thet* (-1 + F1^thet))^2 ))
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      ################### Partial derivatives:
      dHfunc_dTheta <- dvffz(( ((1 + 1/thet) * (log(F1)/F1^thet + log(F2)/F2^thet)/(1/F1^thet + 1/F2^thet - 1)^(1/thet + 2) + log(1/F1^thet + 1/F2^thet - 1)/(thet^2 * (1/F1^thet + 1/F2^thet - 1)^(1 + 1/thet)))/F2^(1 + thet) + (1/F2^(1 + thet) - (1/F2^(1 + thet) + thet * log(F2)/F2^(1 + thet)))/(thet * (1/F1^thet + 1/F2^thet - 1)^(1 + 1/thet)) ))
