...
Clayton270_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  ### GBS: I removed the safeties for: 
  # CDF1, PDF2, HFUNC as well as ALL derivatives. 
  ...
  gradient_margin1_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      HFunc <- (F1^(-thet) + F2^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (F2^((-thet) - 1) * (-thet))) 
      
      ####################
      dHFunc_dF1 <-  F1^(-1 - thet)* F2^(-1 - thet)* (-1 + F1^-thet + F2^-thet)^(-2 - 1/thet) *(1 + thet) 
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2, derlpdf2){
    ...
    expr({
      ...
      ################## Partial derivatives
      dHFunc_dF2 <-  ( (F2^(-2 + thet)* F1^thet* (-1 + F2^-thet + F1^-thet)^(-1/thet)* (-1 + F1^thet)* (1 + thet))/(F1^thet - F2^thet* (-1 + F1^thet))^2 )
      
      dHFunc_dF2 <-  - dHFunc_dF2
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      ################### Partial derivatives:
      dHfunc_dTheta <- ( ((1 + 1/thet) * (log(F1)/F1^thet + log(F2)/F2^thet)/(1/F1^thet + 1/F2^thet - 1)^(1/thet + 2) + log(1/F1^thet + 1/F2^thet - 1)/(thet^2 * (1/F1^thet + 1/F2^thet - 1)^(1 + 1/thet)))/F2^(1 + thet) + (1/F2^(1 + thet) - (1/F2^(1 + thet) + thet * log(F2)/F2^(1 + thet)))/(thet * (1/F1^thet + 1/F2^thet - 1)^(1 + 1/thet)) )
      
      ngr <- ( ( ( (1 - y[,1]) / HFunc ) - (y[,1] / ( pdffz(1 - HFunc) ) ) ) * dHfunc_dTheta * (thet)  )
  ...
  name_fam <- paste("ClaytonCop 270 degree rotation (Mixed binary - continuous margins)", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Rotated Clayton (270°) copula with mixed binary and continuous margins: rho(... link)"                                       
