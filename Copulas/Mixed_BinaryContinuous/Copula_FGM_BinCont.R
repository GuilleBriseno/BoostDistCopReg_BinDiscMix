...
FGM_Cop_BinCont <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_margin1_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      HFunc <- pdffz( F1 * (1 + thet * (1 - 2 * F2) * (1 - F1)) )
      
      ####################
      dHFunc_dF1 <-  1 + thet * (1 - 2 * F1) * (1 - 2 * F2) 
  ...
  name_fam <- paste("FGMCop (Mixed binary - continuous margins)", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "FGM copula with mixed binary and continuous margins: rho(... link)"                                       
