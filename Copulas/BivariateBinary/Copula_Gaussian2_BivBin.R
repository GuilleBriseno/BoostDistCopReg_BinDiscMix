...
Gauss_Cop_BivBinary <- function(marg1 = NULL, marg2 = NULL,
  ...
  #### REMOVED CHECKS IN:  copula parameter, CDFs, and derivatives. Checks will remain in the copula statements!
  ...
  gradient_marg_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      P11 <- pdffz( VGAM:::pbinorm(qnorm(F1), qnorm(F2), cov12 = rho) )
      ...
      derp11_dercdf1 <-  pnorm( (qnorm(F2) - rho * qnorm(F1))/sqrt(1 - rho^2)) 
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2){
    ...
    expr({
      ...
      derp11_dercdf2 <-  pnorm( (qnorm(F1) - rho * qnorm(F2))/sqrt(1 - rho^2)) 
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      derp11_derrho <- VGAM:::dbinorm(qnorm(F1), qnorm(F2), cov12 = !!rho ) 
  ...
  name_fam <- paste("Gaussian copula", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Gauss copula for bivariate binary responses: rho(... link)"                                       
