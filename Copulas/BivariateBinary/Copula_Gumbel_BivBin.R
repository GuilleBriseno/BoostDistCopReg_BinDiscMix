...
Gumbel_Cop_BivBinary <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_marg_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      P11 <- pdffz(  exp( - ( (-log(F1))^thet + (-log(F2))^thet )^(1/thet) ) )
      ...
      derp11_dercdf1 <- dvffz( (exp(-((-log(F1))^thet + (-log(F2))^thet)^((1/thet)))* (-log(F1))^(-1 + thet)* ((-log(F1))^thet + (-log(F2))^thet)^(-1 + 1/thet))/F1 )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2){
    ...
    expr({
      ...
      derp11_dercdf2 <- dvffz(  (exp(-((-log(F2))^thet + (-log(F1))^thet)^((1/thet)))* (-log(F2))^(-1 + thet)* ((-log(F2))^thet + (-log(F1))^thet)^(-1 + 1/thet))/F2  )
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      derp11_derrho <- dvffz(   (1/(thet^2))*exp(-((-log(F1))^thet + (-log(F2))^thet)^((1/thet)))* ((-log(F1))^thet + (-log(F2))^thet)^(-1 + 1/thet)* (-thet* (-log(F1))^thet* log(-log(F1)) + ((-log(F1))^thet + (-log(F2))^thet)* log((-log(F1))^thet + (-log(F2))^thet) - thet *(-log(F2))^thet* log(-log(F2)))  )
  ...
  name_cop <- "Gumbel Copula for bivariate binary responses: rho(... link)"                                       
