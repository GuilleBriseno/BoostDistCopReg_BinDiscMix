...
Frank_Cop_BivBinary <- function(marg1 = NULL, marg2 = NULL,
  ...
  if(!(marg1 %in% c("NO", "SM", "LOGNO", "GA", "DAGUM", "LOGLOG", "EXP", "LOGIT", "PROBIT", "CLOGLOG", "GEVLINK"))) stop("First Marginal distribution not available.")
  if(!(marg2 %in% c("NO", "SM", "LOGNO", "GA", "DAGUM", "LOGLOG", "EXP", "LOGIT", "PROBIT", "CLOGLOG", "GEVLINK"))) stop("Second Marginal distribution not available.")
  ...
  if ( (!is.null(rho) && rho < sqrt(.Machine$double.eps) && rho > sqrt(.Machine$double.eps)) )
    stop(sQuote("rho"), " must be unequal to 0")
  ...
  gradient_marg_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      P11 <- pdffz( -(1/thet)*log( (bit - (1 - exp(-thet*F1))*(1 - exp(-thet*F2)))/bit ) )
      ...
      control_all.p <- rowSums(cbind(P11, P10, P01, P00))
      
      P11 <- P11/control_all.p 
      P10 <- P10/control_all.p
      P01 <- P01/control_all.p
      P00 <- P00/control_all.p
      
      derp11_dercdf1 <- dvffz( (exp(thet)* (-1 + exp(F2* thet)))/(-exp((F1 + F2)* thet) + exp(thet)* (-1 + exp(F1* thet) + exp(F2* thet))) )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2){
    ...
    expr({
      ...
      derp11_dercdf2 <- dvffz( (exp(thet)* (-1 + exp(F1* thet)))/(-exp((F1 + F2)* thet) + exp(thet)* (-1 + exp(F2* thet) + exp(F1* thet))) )
  ...
  # Gradient of likelihood w.r.t. copula parameter
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      derp11_derrho <- dvffz( (exp(thet)* (1/(-1 + exp(thet)) + (-1 - exp(F2* thet)* (-1 + F1) + F1 - exp(F1* thet)* (-1 + F2) + F2)/(exp((F1 + F2)* thet) - exp(thet)* (-1 + exp(F1* thet) + exp(F2* thet))))* thet + log((exp(-(F1 + F2)* thet)* (-exp((F1 + F2)* thet) + exp(thet)* (-1 + exp(F1* thet) + exp(F2* thet))))/(-1 + exp(thet))))/thet^2  )
  ...
  FrankCopFam <- Families(name = name_fam)
  ...
  for(i in marg1$parameter_names){
    ...
    FrankCopFam[[paste(i, "1", sep = "")]] <- fam_obj
  ...
  for(i in marg2$parameter_names){
    ...
    # saving the parameter specific family object in overall families object
    FrankCopFam[[paste(i, "2", sep = "")]] <- fam_obj
  ...
  offset_cop <- expr({
    if (!is.null(rho)){
      ...
      RET <- 0.001 # exp(0) + 1 -> rho = 2
      # ktau <- wdm(x = y[,1], y = y[,2], method = "kendall", weights = w, remove_missing = F)                              # does that work properly with this offset?
      # RET <- log(1/(1-ktau) - 1)
    } 
    return(RET)
  })
  ...
  name_cop <- "Frank copula for bivariate binary responses: rho(... link)"                                       
