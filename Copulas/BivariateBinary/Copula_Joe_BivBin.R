...
Joe_Cop_BivBinary <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_marg_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      P11 <- pdffz( 1 - (bit1 + bit2 - bit1*bit2)^(1/thet) )
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      ngr <-  (
        y[,1]*y[,2]    * ( P11^(-1)                  *   derp11_derrho * exp(f) ) +
          y[,1]*(1-y[,2])    * ( (P10)^(-1) * - derp11_derrho * exp(f) ) +
          (1-y[,1])*y[,2]    * ( (P01)^(-1) * - derp11_derrho * exp(f) ) +
          (1-y[,1])*(1-y[,2]) * ( (P00)^(-1) * derp11_derrho * exp(f) )
      ) 
  ...
  name_cop <- "Joe Copula for bivariate binary responses: rho(... link)"                                       
