...
Clayton180_Cop_BivBinary <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_marg_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      # These two enter always the copula, i.e. are re-defined arguments:
      F1 <- pdffz(1 - !!cdf1)
      ...
      P11 <- pdffz(F1_Tilde + F2_Tilde - 1 + ( F1^-thet + F2^-thet - 1 )^( -1/thet )  )
      ...
      derp11_dercdf1 <- dvffz(1 - derp11_dercdf1)
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2){
    ...
    expr({
      ...
      derp11_dercdf2 <- dvffz(1 - derp11_dercdf2)
  ...
  name_fam <- paste("Clayton copula", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Rotated Clayton (180°) copula for bivariate binary responses: rho(... link)"                                       
