...
Gumbel180_Cop_BivBinary <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_marg_gen <- function(cdf1, cdf2, dercdf1){
    ...
    expr({
      ...
      P11 <- pdffz(F1_Tilde + F2_Tilde - 1 + ( exp( -( (-log(F1) )^(rho) + (-log(F2) )^(rho)  )^(1/(rho)) ) ) )
      ...
      derp11_dercdf1 <- dvffz( (exp(-((-log(F1))^rho + (-log(F2))^rho)^((1/rho)))* (-log(F1))^(-1 + rho)* ((-log(F1))^rho + (-log(F2))^rho)^(-1 + 1/rho))/(F1) )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, dercdf2){
    ...
    expr({
      ...
      derp11_dercdf2 <- dvffz( (exp(-((-log(F2))^rho + (-log(F1))^rho)^((1/rho)))* (-log(F2))^(-1 + rho)* ((-log(F2))^rho + (-log(F1))^rho)^(-1 + 1/rho))/(F2) )
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    ...
    expr({
      ...
      derp11_derrho <- dvffz( (1/((!!rho)^2))*exp(-((-log((F1)))^(!!rho) + (-log((F2)))^(!!rho))^((1/(!!rho))))* ((-log((F1)))^(!!rho) + (-log((F2)))^(!!rho))^(-1 + 1/(!!rho))* (-(!!rho)* (-log((F1)))^(!!rho)* log(-log((F1))) + ((-log((F1)))^(!!rho) + (-log((F2)))^(!!rho))* log((-log((F1)))^(!!rho) + (-log((F2)))^(!!rho)) - (!!rho) *(-log((F2)))^(!!rho)* log(-log((F2))))  )
  ...
  name_cop <- "Rotated Gumbel (180°) Copula for bivariate binary responses: rho(... link)"                                       
