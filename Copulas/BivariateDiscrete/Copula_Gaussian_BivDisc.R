...
source("Marginals/Discrete/Marginal_Poisson.R")
...
Gauss_Cop_BivDiscrete <- function(marg1 = NULL, marg2 = NULL,
  ...
  if(!(marg1 %in% c("POIS", "ZIP", "NBI", "NBII"))) stop("First Marginal distribution not available.")
  if(!(marg2 %in% c("POIS", "ZIP", "NBI", "NBII"))) stop("Second Marginal distribution not available.")
  ...
  if(marg1 == "POIS") {
    marg1 <- Poisson_Mar(loc = 1, offset_mu = mu1)
  }
  
  
  if(marg2 == "POIS") {
    marg2 <- Poisson_Mar(loc = 2, offset_mu = mu2) }
  ...
  gradient_marg_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf1, derpdf1){
    ...
    exprs(
      parse_exprs(paste0("cdf1m1 <-", !!cdf1 - (!!pdf1) )),
      parse_exprs(paste0("cdf2m1 <-", !!cdf2 - (!!pdf2) )),
      parse_exprs(paste0("CDF11  <- expr(VGAM:::pbinorm(qnorm(", !!cdf1, "),  qnorm(", !!cdf2,"), cov12 = ",!!rho,"))" ) ),
      parse_exprs(paste0("CDF01  <- expr(VGAM:::pbinorm(qnorm(cdf1m1),        qnorm(", !!cdf2,"), cov12 = ",!!rho,"))" ) ),
      parse_exprs(paste0("CDF10  <- expr(VGAM:::pbinorm(qnorm(", !!cdf1, "),  qnorm(cdf2m1), cov12 = ",!!rho,"))" ) ),
      parse_exprs(paste0("CDF00  <- expr(VGAM:::pbinorm(qnorm(cdf1m1),        qnorm(cdf2m1), cov12 = ",!!rho,"))" ) ),
    #ngr <-  !!derlpdf1 + rho / (1 - (rho)^2) * (sqrt(2 * pi) / exp(-qnorm( !!cdf1 )^2/2)) * (qnorm( !!cdf2 ) - rho * qnorm( !!cdf1 )) * !!dercdf1
    expr(
      ngr <- (
      ( CDF11 - CDF01 - CDF10 + CDF00 )^(-1) *
      ( (pnorm( (qnorm(!!cdf2) - rho * qnorm(!!cdf1))/sqrt(1 - rho^2)) -                    pnorm( (qnorm(!!cdf2  - !!pdf2) - rho * qnorm(!!cdf1))/sqrt(1 - rho^2)) ) * (!!dercdf1) + 
        (pnorm( (qnorm(!!cdf2  - !!pdf2) - rho * qnorm(!!cdf1 - !!pdf1))/sqrt(1 - rho^2)) - pnorm( (qnorm(!!cdf2) - rho * qnorm(!!cdf1 - !!pdf1))/sqrt(1 - rho^2))  ) * (!!dercdf1 - !!derpdf1) )  
      )
    ),
    expr(return( ngr ))
    )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf2, derpdf2){
    ...
    expr(
      ngr <- (
    ( CDF11 - CDF01 - CDF10 + CDF00  )^(-1) *
    ( ( pnorm( (qnorm(!!cdf1) - rho * qnorm(!!cdf2))/sqrt(1 - rho^2)) -                    pnorm( (qnorm(!!cdf1 - !!pdf1) - rho * qnorm(!!cdf2))/sqrt(1 - rho^2)))  * (!!dercdf2) + 
      ( pnorm( (qnorm(!!cdf1 - !!pdf1) - rho * qnorm(!!cdf2  - !!pdf2))/sqrt(1 - rho^2)) - pnorm( (qnorm(!!cdf1) - rho * qnorm(!!cdf2  - !!pdf2))/sqrt(1 - rho^2))) * (!!dercdf2 - !!derpdf2) )
      )
    ),
    expr(return( ngr ))
    )
  ...
  gradient_cop_gen <- function(cdf1, cdf2, pdf1, pdf2, rho){
    ...
    expr(
      # ngr <- (!!rho / (1 - ( !!rho )^2) + qnorm( !!cdf1 ) * qnorm( !!cdf2 ) * (1 + ( !!rho )^2) / (1 - ( !!rho)^2)^2 -
      #         !!rho * (qnorm( !!cdf1 )^2 + qnorm( !!cdf2 )^2) / (1 - ( !!rho )^2)^2) * (1/cosh(f)^2)
      ngr <-  (
     ( CDF11 - CDF01 - CDF10 + CDF00 )^(-1) *
       ( VGAM:::dbinorm(qnorm(!!cdf1),          qnorm(!!cdf2), cov12 = !!rho ) - 
         VGAM:::dbinorm(qnorm(cdf1m1), qnorm(!!cdf2), cov12 = !!rho ) - 
         VGAM:::dbinorm(qnorm(!!cdf1),          qnorm(cdf2m1), cov12 = !!rho ) + 
         VGAM:::dbinorm(qnorm(cdf1m1), qnorm(cdf2m1), cov12 = !!rho )) * (1/cosh(f)^2)
      )
    ),
    expr(return(ngr))
    )
  ...
  loss_gen <- function(cdf1, cdf2, pdf1, pdf2, rho){
    ...
    # expr(-(log( !!pdf1 ) + log( !!pdf2 ) - 0.5 * log(1 - (!!rho)^2) + (!!rho / (1 - ( !!rho)^2)) * qnorm( !!cdf1 ) * qnorm( !!cdf2 ) -
    #          ((!!rho)^2 / (2*(1 - (!!rho)^2))) * (qnorm( !!cdf1 )^2 + qnorm( !!cdf2)^2)))
    expr( return(- (  log( CDF11 - CDF01 - CDF10 + CDF00 ) ) ) ) 
      )
  }
