...
### Copula function (for bivariate discrete margins)

Clayton90_Cop_BivDiscrete <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_marg_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf1, derpdf1){
    ...
    expr({
      ...
      CDF1m1 <- pdffz(1 - pdffz(F1 - (!!pdf1)))
      
      CDF2m1 <- pdffz(F2 - (!!pdf2))
      
      thet <- rho
      
      T1 <- pdffz(F2 - pdffz( ( F1^-thet + F2^-thet - 1 )^( -1/thet )  ))
      
      T2 <- pdffz(F2 - pdffz( ( CDF1m1^-thet + F2^-thet - 1 )^( -1/thet )  ))
      
      T3 <- pdffz(CDF2m1 - pdffz( ( F1^-thet + CDF2m1^-thet - 1 )^( -1/thet )  ))
      
      T4 <- pdffz(CDF2m1 - pdffz( ( CDF1m1^-thet + CDF2m1^-thet - 1 )^( -1/thet )  ))
      ...
      dT1_dF1 <- dvffz( (F1^(-thet) + F2^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (F1^((-thet) - 1) * (-thet))) )
      
      dT2_dF1 <- dvffz( (CDF1m1^(-thet) + F2^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (CDF1m1^((-thet) - 1) * (-thet))) )
      
      dT3_dF1 <- dvffz( (F1^(-thet) + CDF2m1^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (F1^((-thet) - 1) * (-thet))) )
      
      dT4_dF1 <- dvffz( (CDF1m1^(-thet) + CDF2m1^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (CDF1m1^((-thet) - 1) * (-thet))) )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf2, derpdf2){
    ...
    expr({
      ...
      dT1_dF2 <- dvffz(1 - dvffz( (F1^(-thet) + F2^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (F2^((-thet) - 1) * (-thet))) ))
      
      dT2_dF2 <- dvffz(1 - dvffz( (CDF1m1^(-thet) + F2^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (F2^((-thet) - 1) * (-thet))) ))
      
      dT3_dF2 <- dvffz(1 - dvffz( (F1^(-thet) + CDF2m1^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (CDF2m1^((-thet) - 1) * (-thet))) ))
      
      dT4_dF2 <- dvffz(1 - dvffz( (CDF1m1^(-thet) + CDF2m1^(-thet) - 1)^((-1/thet) - 1) * ((-1/thet) * (CDF2m1^((-thet) - 1) * (-thet))) ))
  ...
  gradient_cop_gen <- function(cdf1, cdf2, pdf1, pdf2, rho){
    ...
    expr({
      ...
      dT1_dtheta <- dvffz((-1)*( ((-1 + F1^-thet + F2^-thet)^(-1/thet) *((thet *(F2^thet *log(F1) + F1^thet* log(F2)))/(F2^thet - F1^thet* (-1 + F2^thet)) + log(-1 + F1^-thet + F2^-thet)))/thet^2  ))
      
      dT2_dtheta <- dvffz( (-1)*(((-1 + CDF1m1^-thet + F2^-thet)^(-1/thet) *((thet *(F2^thet *log(CDF1m1) + CDF1m1^thet* log(F2)))/(F2^thet - CDF1m1^thet* (-1 + F2^thet)) + log(-1 + CDF1m1^-thet + F2^-thet)))/thet^2 ) )
      
      dT3_dtheta <- dvffz( (-1)*(((-1 + F1^-thet + CDF2m1^-thet)^(-1/thet) *((thet *(CDF2m1^thet *log(F1) + F1^thet* log(CDF2m1)))/(CDF2m1^thet - F1^thet* (-1 + CDF2m1^thet)) + log(-1 + F1^-thet + CDF2m1^-thet)))/thet^2 ) )
      
      dT4_dtheta <- dvffz( (-1)*(((-1 + CDF1m1^-thet + CDF2m1^-thet)^(-1/thet) *((thet *(CDF2m1^thet *log(CDF1m1) + CDF1m1^thet* log(CDF2m1)))/(CDF2m1^thet - CDF1m1^thet* (-1 + CDF2m1^thet)) + log(-1 + CDF1m1^-thet + CDF2m1^-thet)))/thet^2 ) )
  ...
  name_fam <- paste("ClaytonCop 90 degree rotation", marg1$marg_name, marg2$marg_name)
  ...
  name_cop <- "Rotated Clayton (90°) copula for bivariate discrete / count responses: rho(... link)"                                       
