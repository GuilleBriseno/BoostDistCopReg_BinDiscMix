...
### FRANK-Copula function (for bivariate binary margins)

Frank_Cop_BivDiscrete <- function(marg1 = NULL, marg2 = NULL,
  ...
  if ((!is.null(rho) && abs(rho) < c(sqrt(.Machine$double.eps)) ))
    stop(sQuote("rho"), " must not be 0.")
  ...
  gradient_marg_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf1, derpdf1){
    ...
    expr({
      ...
      T1 <- ( -(1/thet)*log( (bit - (1 - exp(-thet*F1))*(1 - exp(-thet*F2)))/bit ) )
      
      T2 <- ( -(1/thet)*log( (bit - (1 - exp(-thet*CDF1m1))*(1 - exp(-thet*F2)))/bit )  ) 
      
      T3 <- ( -(1/thet)*log( (bit - (1 - exp(-thet*F1))*(1 - exp(-thet*CDF2m1)))/bit ) )
      
      T4 <- ( -(1/thet)*log( (bit - (1 - exp(-thet*CDF1m1))*(1 - exp(-thet*CDF2m1)))/bit ) )
      ...
      dT1_dF1 <- pdffz( (exp(thet)* (-1 + exp(F2* thet)))/(-exp((F1 + F2)* thet) + exp(thet)* (-1 + exp(F1* thet) + exp(F2* thet))) )
      
      dT2_dF1 <- pdffz( (exp(thet)* (-1 + exp(F2* thet)))/(-exp((CDF1m1 + F2)* thet) + exp(thet)* (-1 + exp(CDF1m1* thet) + exp(F2* thet))) )
      
      dT3_dF1 <- pdffz( (exp(thet)* (-1 + exp(CDF2m1* thet)))/(-exp((F1 + CDF2m1)* thet) + exp(thet)* (-1 + exp(F1* thet) + exp(CDF2m1* thet))) )
      
      dT4_dF1 <- pdffz( (exp(thet)* (-1 + exp(CDF2m1* thet)))/(-exp((CDF1m1 + CDF2m1)* thet) + exp(thet)* (-1 + exp(CDF1m1* thet) + exp(CDF2m1* thet))) )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf2, derpdf2){
    ...
    expr({
      ...
      dT1_dF2 <- pdffz( (exp(thet)* (-1 + exp(F1* thet)))/(-exp((F1 + F2)* thet) + exp(thet)* (-1 + exp(F2* thet) + exp(F1* thet))) )
      
      dT2_dF2 <- pdffz( (exp(thet)* (-1 + exp(CDF1m1* thet)))/(-exp((CDF1m1 + F2)* thet) + exp(thet)* (-1 + exp(F2* thet) + exp(CDF1m1* thet))) )
      
      dT3_dF2 <- pdffz( (exp(thet)* (-1 + exp(F1* thet)))/(-exp((F1 + CDF2m1)* thet) + exp(thet)* (-1 + exp(CDF2m1* thet) + exp(F1* thet))) )
      
      dT4_dF2 <- pdffz(  (exp(thet)* (-1 + exp(CDF1m1* thet)))/(-exp((CDF1m1 + CDF2m1)* thet) + exp(thet)* (-1 + exp(CDF2m1* thet) + exp(CDF1m1* thet)))  )
  ...
  gradient_cop_gen <- function(cdf1, cdf2, pdf1, pdf2, rho){
    ...
    expr({
      ...
      dT1_dtheta <-  (exp(thet)* (1/(-1 + exp(thet)) + (-1 - exp(F2* thet)* (-1 + F1) + F1 - exp(F1* thet)* (-1 + F2) + F2)/(exp((F1 + F2)* thet) - exp(thet)* (-1 + exp(F1* thet) + exp(F2* thet))))* thet + log((exp(-(F1 + F2)* thet)* (-exp((F1 + F2)* thet) + exp(thet)* (-1 + exp(F1* thet) + exp(F2* thet))))/(-1 + exp(thet))))/thet^2  
      
      dT2_dtheta <-  (exp(thet)* (1/(-1 + exp(thet)) + (-1 - exp(F2* thet)* (-1 + CDF1m1) + CDF1m1 - exp(CDF1m1* thet)* (-1 + F2) + F2)/(exp((CDF1m1 + F2)* thet) - exp(thet)* (-1 + exp(CDF1m1* thet) + exp(F2* thet))))* thet + log((exp(-(CDF1m1 + F2)* thet)* (-exp((CDF1m1 + F2)* thet) + exp(thet)* (-1 + exp(CDF1m1* thet) + exp(F2* thet))))/(-1 + exp(thet))))/thet^2  
      
      dT3_dtheta <-  (exp(thet)* (1/(-1 + exp(thet)) + (-1 - exp(CDF2m1* thet)* (-1 + F1) + F1 - exp(F1* thet)* (-1 + CDF2m1) + CDF2m1)/(exp((F1 + CDF2m1)* thet) - exp(thet)* (-1 + exp(F1* thet) + exp(CDF2m1* thet))))* thet + log((exp(-(F1 + CDF2m1)* thet)* (-exp((F1 + CDF2m1)* thet) + exp(thet)* (-1 + exp(F1* thet) + exp(CDF2m1* thet))))/(-1 + exp(thet))))/thet^2 
      
      dT4_dtheta <-   (exp(thet)* (1/(-1 + exp(thet)) + (-1 - exp(CDF2m1* thet)* (-1 + CDF1m1) + CDF1m1 - exp(CDF1m1* thet)* (-1 + CDF2m1) + CDF2m1)/(exp((CDF1m1 + CDF2m1)* thet) - exp(thet)* (-1 + exp(CDF1m1* thet) + exp(CDF2m1* thet))))* thet + log((exp(-(CDF1m1 + CDF2m1)* thet)* (-exp((CDF1m1 + CDF2m1)* thet) + exp(thet)* (-1 + exp(CDF1m1* thet) + exp(CDF2m1* thet))))/(-1 + exp(thet))))/thet^2  
      
      
      ########## FINAL GRADIENT
      ngr <-  ( ( L_Total )^(-1) * ( dT1_dtheta - dT2_dtheta - dT3_dtheta + dT4_dtheta ) * 1 )
  ...
  name_fam <- paste("FrankCop", marg1$marg_name, marg2$marg_name)
  ...
  offset_cop <- expr({
    if (!is.null(rho)){
      ...
      temp <- ifelse( abs(rho) < c(sqrt(.Machine$double.eps)), c(sqrt(.Machine$double.eps)), rho )
      
      RET <- rho 
      
    } else {
      RET <- 0.001 # rho = 0/(sqrt(1 + 0)) = 0
      #RET <- rep(0, nrow(y))
  ...
  response_cop <- function(f) f
  
  check_y_cop <- expr({
    y
  })
  
  name_cop <- "Frank copula for bivariate discrete / count responses: rho(... link)"                                       
