...
Gauss_Cop_BivDiscrete <- function(marg1 = NULL, marg2 = NULL,
  ...
  if(!(marg1 %in% c("POIS", "ZIP", "NBI", "NBII", "GEOM", "ZALG", "ZINBI", "ZANBI", "PIG", "ZIPIG"))) stop("First Marginal distribution not available.")
  if(!(marg2 %in% c("POIS", "ZIP", "NBI", "NBII", "GEOM", "ZALG", "ZINBI", "ZANBI", "PIG", "ZIPIG"))) stop("Second Marginal distribution not available.")
  ...
  if(marg1 == "POIS") {
    ...
    #marg1 <- Poisson_Mar(loc = 1, offset_mu = mu1, aux_mat = NULL, dim_aux_mat = NULL)
    marg1 <- Poisson_Mar(loc = 1, offset_mu = mu1, aux_mat = auxiliary_material[[1]], dim_aux_mat = dim(auxiliary_material[[1]]))
    ...
    marg1 <- ZIP_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1)
  }  else if(marg1 == "NBI") {
    ...
    #marg1 <- ZALG_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1, aux_mat = auxiliary_material[[1]], dim_aux_mat = dim(auxiliary_material[[1]]))
    
  }  else if(marg1 == "ZINBI") {
    
    marg1 <- ZINBI_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1, offset_nu = nu1, aux_mat = NULL, dim_aux_mat = NULL)
    #marg1 <- ZINBI_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1, offset_nu = nu1, auxiliary_material[[1]], dim_aux_mat = dim(auxiliary_material[[1]]))
    
  } else if(marg1 == "ZANBI") {
    
    marg1 <- ZANBI_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1, offset_nu = nu1, aux_mat = NULL, dim_aux_mat = NULL)
    #marg1 <- ZANBI_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1, offset_nu = nu1, aux_mat = auxiliary_material[[1]], dim_aux_mat = dim(auxiliary_material[[1]]))
    
  } else if(marg1 == "PIG") {
    marg1 <- PIG_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1, aux_mat = NULL, dim_aux_mat = NULL)
  } else if(marg1 == "ZIPIG") {
    marg1 <- ZIPIG_Mar(loc = 1, offset_mu = mu1, offset_sigma = sigma1, offset_nu = nu1, aux_mat = NULL, dim_aux_mat = NULL)
  } 
  ...
  if(marg2 == "POIS") {
    ...
    #marg2 <- Poisson_Mar(loc = 2, offset_mu = mu2, aux_mat = NULL, dim_aux_mat = NULL) 
    marg2 <- Poisson_Mar(loc = 2, offset_mu = mu2, aux_mat = auxiliary_material[[2]], dim_aux_mat = dim(auxiliary_material[[2]]))
    ...
    marg2 <- ZIP_Mar(loc = 2, offset_mu = mu2, offset_sigma = sigma2)
  }else if(marg2 == "NBI") {
    ...
    #marg2 <- ZALG_Mar(loc = 2, offset_mu = mu2, offset_sigma = sigma2, aux_mat = auxiliary_material[[2]], dim_aux_mat = dim(auxiliary_material[[2]]))
    
  }  else if(marg2 == "ZINBI") {
    
    marg2 <- ZINBI_Mar(loc = 2, offset_mu = mu2, offset_sigma = sigma2, offset_nu = nu2, aux_mat = NULL, dim_aux_mat = NULL)
    #marg2 <- ZINBI_Mar(loc = 2, offset_mu = mu2, offset_sigma = sigma2, offset_nu = nu2, aux_mat = auxiliary_material[[2]], dim_aux_mat = dim(auxiliary_material[[2]]))
    
  } else if(marg2 == "ZANBI") {
    
    marg2 <- ZANBI_Mar(loc = 2, offset_mu = mu2, offset_sigma = sigma2, offset_nu = nu2, aux_mat = NULL, dim_aux_mat = NULL)
    #marg2 <- ZANBI_Mar(loc = 2, offset_mu = mu2, offset_sigma = sigma2, offset_nu = nu2, aux_mat = auxiliary_material[[2]], dim_aux_mat = dim(auxiliary_material[[2]]))
    ...
    marg2 <- ZIPIG_Mar(loc = 2, offset_mu = mu2, offset_sigma = sigma2, offset_nu = nu2, aux_mat = NULL, dim_aux_mat = NULL)
  } 
  ...
  rho_simp <- expr(rho)
  rho_gen <- expr(tanh(f))
  #rho_gen <- expr({  tanh( check_eta_theta(f, coptype = "GAUSS") ) })
  ...
  gradient_marg_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf1, derpdf1){
    ...
    expr({
      ...
      T1 <- pdffz(VGAM:::pbinorm(qnorm(F1),    qnorm(F2), cov12 = rho))
      
      T2 <- pdffz(VGAM:::pbinorm(qnorm(CDF1m1),    qnorm(F2), cov12 = rho))
      
      T3 <- pdffz(VGAM:::pbinorm(qnorm(F1),    qnorm(CDF2m1), cov12 = rho))
      
      T4 <- pdffz(VGAM:::pbinorm(qnorm(CDF1m1),    qnorm(CDF2m1), cov12 = rho))
      ...
      dT1_dF1 <- dvffz( pnorm( (qnorm(F2) - rho * qnorm(F1))/sqrt(1 - rho^2)) )
      
      dT2_dF1 <- dvffz( pnorm( (qnorm(CDF2m1) - rho * qnorm(F1))/sqrt(1 - rho^2)) )
      
      dT3_dF1 <- dvffz( pnorm( (qnorm(F2) - rho * qnorm(CDF1m1))/sqrt(1 - rho^2)) )
      
      dT4_dF1 <- dvffz(  pnorm( (qnorm(CDF2m1) - rho * qnorm(CDF1m1))/sqrt(1 - rho^2)) ) 
      
      ngr <- ( ( L_Total )^(-1) * ( ( dT1_dF1 - dT2_dF1 ) * (dF1_deta) + ( - dT3_dF1 + dT4_dF1  ) * (dF1_deta - dpdf1_deta) )  )
  ...
  gradient_margin2_gen <- function(cdf1, cdf2, pdf1, pdf2, dercdf2, derpdf2){
    ...
    expr({
      ...
      dT1_dF2 <- dvffz( pnorm( (qnorm(F1) - rho * qnorm(F2))/sqrt(1 - rho^2)) )
      
      dT2_dF2 <- dvffz( pnorm( (qnorm(CDF1m1) - rho * qnorm(F2))/sqrt(1 - rho^2)) )
      
      dT3_dF2 <- dvffz( pnorm( (qnorm(F1) - rho * qnorm(CDF2m1))/sqrt(1 - rho^2)) )
      
      dT4_dF2 <- dvffz( pnorm( (qnorm(CDF1m1) - rho * qnorm(CDF2m1))/sqrt(1 - rho^2)) ) 
  ...
  gradient_cop_gen <- function(cdf1, cdf2, pdf1, pdf2, rho){
    ...
    expr({
      ...
      dT1_dtheta <- VGAM:::dbinorm(qnorm(F1), qnorm(F2), cov12 = !!rho )
      
      dT2_dtheta <- VGAM:::dbinorm(qnorm(CDF1m1),   qnorm(F2), cov12 = !!rho )
      
      dT3_dtheta <- VGAM:::dbinorm(qnorm(F1),   qnorm(CDF2m1), cov12 = !!rho )
      
      dT4_dtheta <- VGAM:::dbinorm(qnorm(CDF1m1),   qnorm(CDF2m1), cov12 = !!rho )
      
      
      ########## FINAL GRADIENT
      ngr <-  ( ( L_Total )^(-1) * ( dT1_dtheta - dT2_dtheta - dT3_dtheta + dT4_dtheta ) * (1/cosh(f)^2)
      )
  ...
  name_cop <- "Gauss copula for bivariate discrete / count responses: rho(... link)"                                       
